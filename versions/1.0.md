...
- **location** `Location Object` - Geographic coordinates and timezone information (optional when using static CSV files)
- **calculationMethod** `CalculationMethod Object` - Parameters used for prayer time calculations (optional when using static CSV files)
- **dailyPrayerTimes** `DailyPrayerTimes Object` - Contains the reference to the static CSV prayer times data (optional when using dynamic calculation methods)

Example document with static CSV file reference:
...
Example document with calculation metadata and static CSV file used to overwrite calculations for specific dates:
...
### 3.2. SalahAPI CSV File Structure

The SalahAPI CSV file contains daily prayer times data in a structured format. The first row contains the headers, and subsequent rows contain the prayer times for each date.
...
Provides a reference to the static CSV file containing daily prayer times.

**Fields:**

- **csvUrl** `string` **REQUIRED** - URL to the static CSV file containing prayer times data
- **dateFormat** `string` **REQUIRED** - Date format used in the CSV file (e.g., "YYYY-MM-DD" for ISO 8601 or "DD/MM/YYYY" for common formats)
- **timeFormat** `string` **REQUIRED** - Time format used in the CSV file (e.g., "HH:mm" for 24-hour format or "hh:mm A" for 12-hour format)