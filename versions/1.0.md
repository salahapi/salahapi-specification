...
- [4. Data Structures](#4-data-structures)
  ...
  - [Iqama Calculation Rules Object](#iqama-calculation-rules-object)
  - [Prayer Calculation Rule Object](#prayer-calculation-rule-object)
  - [Jumuah Rule Object](#jumuah-rule-object)
  - [Jumuah Location Object](#jumuah-location-object)
...
The Contact Object specifies contact information for the service maintainer.

**Fields:**

- **name** `string` - **OPTIONAL**. The identifying name of the contact person or organization.
- **email** `string` - **OPTIONAL**. The email address of the contact person or organization. The value MUST be in the format of an email address.
...
The `IqamaCalculationRules` Object specifies the rules for calculating Iqama (congregation prayer start time) times.
...
The `PrayerCalculationRule` Object specifies a single prayer time calculation rule.
...
- **overrides** `array of PrayerCalculationOverrideRule` - **OPTIONAL**. An array of date-specific overrides for the prayer time calculation rule.

### PrayerCalculationOverrideRule Object

The `PrayerCalculationOverrideRule` Object specifies a date-specific override for a prayer time calculation rule.

**Fields:**

- **condition** `enum` - **REQUIRED**. The type of override rule. Value MUST be one of: "daylightSavingsTime", "standardTime", "dateRange".
- **fromDate** `string` - **REQUIRED if condition is "dateRange"**. The start date of the override in the format specified by the `dateFormat` field.
- **toDate** `string` - **REQUIRED if condition is "dateRange"**. The end date of the override in the format specified by the `dateFormat` field
- **time** `PrayerCalculationRule Object` - **REQUIRED**. The prayer calculation rule to apply for the override.

### JumuahRule Object

The `JumuahRule` Object specifies a single Jumuah (Friday prayer) rule.
...
The `JumuahLocation` Object specifies the location for a Jumuah prayer.
...
The `DailyPrayerTimes` Object provides a reference to the CSV data containing daily prayer times.
...
The `CsvUrlParameters` Object specifies optional URL parameters that MAY be passed to the CSV endpoint.
